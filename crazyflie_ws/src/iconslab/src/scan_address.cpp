	...
	try {
		...
		if (numCrazyradios > 0) {
			...
			for (uint8_t datarate = 0; datarate < 3; ++datarate) {
				...
				for (uint32_t addressOff = 0; addressOff <= 255; addressOff++) {
					uint64_t finalAddress = address + addressOff;
					radio.setAddress(finalAddress);
					...
					if (ack.ack) {
						...
						std::cout << "/" << std::uppercase << std::hex << (address + addressOff) << std::endl;
					}
				}
			}
