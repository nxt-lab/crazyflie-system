...
class Controller():
    def __init__(self, use_controller, joy_topic):
        ...
        if use_controller:
            ...
            self._land0 = rospy.ServiceProxy('land0', Empty)
            #self._land1 = rospy.ServiceProxy('land1', Empty)

            rospy.loginfo("waiting for takeoff service")
            rospy.wait_for_service('takeoff')
            rospy.loginfo("found takeoff service")
            self._takeoff0 = rospy.ServiceProxy('takeoff0', Empty)
            #self._takeoff1 = rospy.ServiceProxy('takeoff1', Empty)
        else:
            self._land0 = None
            #self._land1 = None
            self._takeoff0 = None
    ...
    def _joyChanged(self, data):
        for i in range(0, len(data.buttons)):
            if self._buttons == None or data.buttons[i] != self._buttons[i]:
                if i == 0 and data.buttons[i] == 1 and self._land0 != None:
                    self._land0()
                    #self._land1()
                if i == 1 and data.buttons[i] == 1:
                    self._emergency()
                if i == 2 and data.buttons[i] == 1 and self._takeoff0 != None:
                    self._takeoff0()
                    #self._takeoff1()
